...
/// @title  ScalarResolverV7
/// @notice Generalized, stake-weighted, commit-reveal Schelling resolver for
///         ARBITRARY intersubjective outcomes. Returns a CONTINUOUS score in
///         [0, 10000] (not a binary verdict, not a 4-bucket step function),
///         which the bound market splits escrow by proportionally.
///
///         v0.7 adds the protocol's headline mechanism-design property:
///         **STAKER-PARTICIPANT DECOUPLING.** The parties to a market (its
///         service and agent), as registered by the bound market contract,
///         are barred from voting on that market. This is the structural fix
///         optimistic-oracle incumbents cannot close: in UMA the
///         settlement/governance token IS the betting token; in coupled-stake
///         designs the judge and the participant are the same capital. Here a
///         market's own participants can never be its resolvers.
///
///         All prior mechanics are preserved: commit-reveal sealed voting
///         (anti-copy), stake-weighted median with a 40% voting-weight cap,
///         distance-from-median slashing, subscription pool (MasterChef
///         accumulator), ERC-8004 reputation events, 7-day unstake cooldown,
///         pendingVotes guard, dispute reward pool.
///
/// @dev    Decoupling is enforced at COMMIT time. The bound market contract
///         MUST call `registerConflict` before the resolver's commit window
///         opens for a market. In Crucible the market registers (service,
///         agent) inside `dispute()`, which always precedes the first
///         `commitVote` (windows bootstrap on first commit). A conflicted
///         address can therefore never enter the voter set, so resolution is
///         computed only over non-participant validators.
///
...
///
///         A market with zero revealed votes after revealDeadline reverts in
///         resolve() with NoVotes; the market contract exposes a
///         force-resolve-default path for that case.
///
/// @dev    pendingVotes is incremented on REVEAL (not commit), because only
///         revealed votes commit the validator to a market outcome.
contract ScalarResolverV7 is IResolver, IResolverFeeReceiver, IResolverSubscriptionReceiver, ReentrancyGuard {
    ...
    ///         in this market and may not vote on it (staker-participant
    ///         decoupling). Set only by AUTHORIZED_MARKET via registerConflict.
    mapping(bytes32 => mapping(address => bool)) public conflicted;

    constructor(uint256 _minStake, address _authorizedMarket) {
    ...
    function onDispute(bytes32 marketId, address a, address b) external {
        ...
        if (a != address(0) && !conflicted[marketId][a]) {
            conflicted[marketId][a] = true;
            emit ConflictRegistered(marketId, a);
        }
        if (b != address(0) && !conflicted[marketId][b]) {
            conflicted[marketId][b] = true;
            emit ConflictRegistered(marketId, b);
        }
    }
    ...
    function commitVote(bytes32 marketId, bytes32 voteHash) external {
        if (conflicted[marketId][msg.sender]) revert ConflictedParty();
