...
/// @title  ScalarResolverV9
/// @notice **CALIBRATION-WEIGHTED consensus** (headline innovation) on top of
///         ScalarResolverV8 (ERC-8004 identity binding) and V7 (decoupling +
///         window-denial fix). A validator's vote weight = stake scaled by an
...
contract ScalarResolverV9 is IResolver, IResolverFeeReceiver, IResolverSubscriptionReceiver, ReentrancyGuard {
    ...
    uint256 public constant CALIB_STEP  = 1000;   // per-market adjustment
    ...
    constructor(uint256 _minStake, address _authorizedMarket, address _identityRegistry) {
    ...
    function _updateCalibration(address v, bool honest, bytes32 marketId) internal {
        ...
        if (honest) {
            ...
            nc = c <= CALIB_MIN + CALIB_STEP ? CALIB_MIN : c - CALIB_STEP;
        }
        calibration[v] = nc;
    ...
    function resolve(bytes32 marketId, bytes calldata) external returns (uint256) {
        ...
        for (uint256 i = 0; i < len;) {
            address v = vs[i];
            uint256 distance = _abs(int256(uint256(votes[marketId][v])) - int256(uint256(finalScoreBps)));
            _updateCalibration(v, distance <= TOLERANCE_BPS, marketId);
            unchecked { ++i; }
        }
