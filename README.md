...
[![Tests](https://img.shields.io/badge/tests-185%2F185%20passing-success)](#)
...
```
8 protocol versions shipped (v0 → v0.7 + V8/V9 resolvers), each a deployable artifact
185 forge tests + 7 SDK tests passing
v0.7 live on Arc Testnet — graded-resolution layer (V7/V8/V9 resolvers deployed)
...
Deployment txs: market [`0x6ba571a3…`](https://testnet.arcscan.app/tx/0x6ba571a3e940cc4465a7e5ff73e32f45c19781724fb1f2bf5f7c733609ead983) · resolver [`0xd8113bc8…`](https://testnet.arcscan.app/tx/0xd8113bc89004e0316288251c1dd72452b816abbc0efcc7ba27071221811c7720) · adapter [`0xc92706c4…`](https://testnet.arcscan.app/tx/0xc92706c44645ab275d1b160f4a26603fc078459370e2d59f7f9961f895116cec) · V9 [`0xb5868bea…`](https://testnet.arcscan.app/tx/0xb5868beafc9bdecc460e4119f2ea463a21706882f9c40be90864877a1ad5d7b0)
...
**M2/M3 since:** **ERC-8004 identity-level decoupling** (`ScalarResolverV8` — bars every address a participant's *identity* controls) and **calibration-weighted consensus** (`ScalarResolverV9` — `voteWeight = stake × earned-accuracy calibration`, 0.25×…1.50×, so fresh capital is worth half a proven validator's; a self-generated on-chain reputation that needs no external ecosystem). Honestly scoped: calibration is a *bounded tilt* toward accuracy, not a whale defense. Full write-up: [`docs/repositioning-v0.7.md`](docs/repositioning-v0.7.md) · Grant draft: [`docs/grant-application-v0.7.md`](docs/grant-application-v0.7.md).
...
- **Pre-audit.** 185 forge tests pass, but no independent security audit. Treat as testnet-only research code.
...
- **Calibration is farmable in the bootstrap phase (disclosed, demonstrated, bounded).** Because calibration rewards a track record of voting-with-consensus, a cartel can *manufacture* that record by voting with itself on throwaway markets. `test_limitation_farmedCartelBeatsFreshHonest` proves a 2-validator cartel farmed to the 1.50× ceiling can override **four** equal-stake *fresh* honest validators while holding **half** the stake. Why it is bounded, not fatal: (1) farming costs real USDC — every market needs escrow + a dispute bond; (2) the edge **vanishes** once honest validators have themselves accrued calibration (it is a bootstrap-phase asymmetry, not a permanent one); (3) the durable fix is **value-weighted calibration gain** (scale the per-market step by the resolved escrow, so dust-sized throwaway markets earn ≈ nothing) and/or an asymmetric slow-rise / fast-fall step — both future work. We ship the demonstrated limitation rather than hide it: the headline (proven-honest beats fresh) and its flip side (proven-cartel beats fresh) are the *same* mechanism, and honesty about that is the point.
